...
ret_limit = 0.04 # test spy drop limit
result = pd.DataFrame()
...
result['spy_ret_limit_gain_fl'] = result[str(ret_days)+'d_max_gain'].apply(lambda x: 1 if x<ret_limit else 0)
...
result.to_excel("spy_test_gain_result.xlsx")
test_result.to_excel("spy_predict_gain_result.xlsx")
