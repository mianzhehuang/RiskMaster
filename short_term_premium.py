# -*- coding: utf-8 -*-
"""
Created on Thu Feb  4 12:40:41 2021
...
ret_days = 4 # test spy xx days return
ret_limit = -0.02 # test spy drop limit
result = pd.DataFrame()
...
#strike_distance['date'] = strike_distance['date'].apply(lambda x: datetime.datetime.strptime(x, "%m/%d/%Y"))
#strike_distance.set_index("date", inplace=True)

#strike_distance['1M20DP'] = strike_distance['1M20DP'].apply(lambda x: float(x))
result = pd.concat([result, max_drop(spy_hist, ret_days)], axis=1)
#result = pd.concat([result, strike_distance[['1M20DP']]], axis=1) 
result['spy_ret_limit_loss_fl'] = result.apply(lambda x: 1 if x[str(ret_days)+'d_max_loss']>(ret_limit) else 0, axis=1)
...
#%% Calculate the VWAP
result = pd.concat([result, vwap_delta(spy_hist,7)], axis =1)
...
#                      'adx_7', 'adx_14']]
#
#result = result[['4d_max_loss', 'spy_ret_limit_loss_fl',
#                 'vix_current', 'vix_sma_20',
#                  'vol_20_day', #'vol_100_day',
#                  'sma_10_diff', 'sma_20_diff', #'sma_100_diff',
#                  'cci_7', 'cci_14', #'cci_40',
#                  'cmo_7', 'cmo_14',
#                  'adx_7', 'adx_14', 'vwap_delta_7']]


#%% 
result_new = pd.DataFrame()
result.reset_index(inplace=True)
#test_result_new = pd.DataFrame()

for i in result.index:
    if (i % (ret_days-1)) == 0:
        result_new = pd.concat([result_new, result[result.index==i]])


result_new.set_index('index', inplace=True)

test_result = result[['index', 'vix_current', 'vix_sma_20',
                  'vol_20_day', #'vol_100_day',
                  'sma_10_diff', 'sma_20_diff', #'sma_100_diff',
                  'cci_7', 'cci_14', #'cci_40',
                  'cmo_7', 'cmo_14',
...
result_new = result_new[['4d_max_loss', 'spy_ret_limit_loss_fl',
                 'vix_current', 'vix_sma_20',
                  'vol_20_day', #'vol_100_day',
                  'sma_10_diff', 'sma_20_diff', #'sma_100_diff',
                  'cci_7', 'cci_14', #'cci_40',
...
result_new.dropna(inplace=True)
test_result.dropna(inplace=True)   
test_result.set_index('index', inplace=True)     

result.to_excel("spy_test_short_term.xlsx")
test_result.to_excel("spy_predict_short_term.xlsx")
