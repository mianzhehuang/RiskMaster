# -*- coding: utf-8 -*-
"""
Created on Tue Jan 19 13:37:28 2021
...
vxx_hist = yf.download('VXX', start= start, end=end)


#%% Construct the Y-variable
result = pd.concat([result, max_gain(spy_hist, ret_days)], axis=1) 
strike_distance['Index'] = strike_distance['date'].apply(lambda x: datetime.datetime.strptime(x, '%m/%d/%Y'))
strike_distance_spec = strike_distance[['Index', '1M10DC']]
strike_distance_spec.set_index('Index', inplace=True)


result = pd.concat([result, strike_distance_spec], axis=1)
result = result.dropna()


result['spy_ret_limit_gain_fl'] = result.apply(lambda x: 1 if x[str(ret_days)+'d_max_gain']<x['1M10DC']-0.01 else 0, axis=1)
...
result = result[['22d_max_gain', 'spy_ret_limit_gain_fl',
...
result.to_excel(r"C:\Users\huang\Dropbox\Carrick Huang\riskmaster\spy_test_gain_result.xlsx")
test_result.to_excel(r"C:\Users\huang\Dropbox\Carrick Huang\riskmaster\spy_predict_gain_result.xlsx")
