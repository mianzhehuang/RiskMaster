# -*- coding: utf-8 -*-
"""
Created on Sun Sep 12 16:57:05 2021
...
ret_days = 10 # test spy xx days return
ret_limit = 0.03 # test spy drop limit
result = pd.DataFrame()
#strike_distance = pd.read_csv(r"C:\Users\huang\Dropbox\Algo Force\Signal\Strike Distances.csv")
gld_hist = yf.download('GLD', start= start, end=end)
vix_hist = yf.download('^VIX', start= start, end=end)

drop_window_start = '2021-01-01'
drop_window_end   = '2021-01-31'
timestamp_list = pd.date_range(start=drop_window_start,end=drop_window_end, periods = len(pd.date_range(start=drop_window_start,end=drop_window_end)))
#%% Construct the Y-variable
result = pd.concat([result, max_gain(gld_hist, ret_days)], axis=1) 


result['gld_ret_limit_gain_fl'] = result[str(ret_days)+'d_max_gain'].apply(lambda x: 1 if x<ret_limit else 0)

#%% Contruct the X-variable for SMA
result = pd.concat([result, sma_diff(gld_hist, 10)], axis=1) 
result = pd.concat([result, sma_diff(gld_hist, 20)], axis=1)
result = pd.concat([result, sma_diff(gld_hist, 100)], axis=1)
...
result = pd.concat([result, std_vol(gld_hist, 20)], axis=1) 
result = pd.concat([result, std_vol(gld_hist, 100)], axis=1) 

#%% Construct CCI level
result = pd.concat([result, cci(gld_hist, 7)], axis=1) 
result = pd.concat([result, cci(gld_hist, 14)], axis=1) 
result = pd.concat([result, cci(gld_hist, 40)], axis=1) 

#%% Construct CCI level
result = pd.concat([result, cmo(gld_hist, 7)], axis=1) 
result = pd.concat([result, cmo(gld_hist, 14)], axis=1)  

#%% Contruct CCI ADX level
result = pd.concat([result, adx(gld_hist,7)], axis =1)
result = pd.concat([result, adx(gld_hist,14)], axis =1)
...
result = result[['10d_max_gain', 'gld_ret_limit_gain_fl',
...
result = result.drop(timestamp_list, errors='ignore')
result.to_excel("gld_test_gain_result.xlsx")
test_result.to_excel("gld_predict_gain_result.xlsx")